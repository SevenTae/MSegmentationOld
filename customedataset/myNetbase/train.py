...
from customedataset2.myNetbase.evaluate_train import evalue_iou_miou, evaluateloss
import time
from nets.myNetbase.su_model import SUNet, weights_init
import numpy  as np
import  os
'''Tree数据集'''
...
# 加了tensorboard记录
dir_checkpoint = Path('./checkpoints0/')
tensorboarddir = Path('./TensorboardLog0/')
...
def train_net(net,
    ...
    argsval.resize = (512, 512)  # 输入是w h(长和高)
    imgshape_base = [512, 512]
    valissuo = False  # 测试的时候是否缩放 #注意这里测试和预测集的标签是原图大小
    train_d = pascal_customer2.Customer_VOCSegmentation(argstrain, split='train', isAug=True)
    val_d = pascal_customer2.Customer_VOCSegmentation(argsval, split='val')

    n_val = val_d.__len__()
    n_train = train_d.__len__()

    train_loader = DataLoader(train_d, batch_size=train_batch_size, shuffle=True, num_workers=2, pin_memory=False,
                              drop_last=True)
    val_loader = DataLoader(val_d, shuffle=True, num_workers=2, pin_memory=False, batch_size=val_batch_size) #验证机不可以shuffle否则每次验证的时候结果都不一样

    # (Initialize logging)
    experiment = wandb.init(project='dataset1', resume='allow', anonymous='must')
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        isevalue = True
        if isevalue == True:
            acc_global, acc, iu, miou = evalue_iou_miou(net, val_loader, device, args.classes, )
            val_score = miou
...
def get_args():
    ...
    parser.add_argument('--val_batch-size', '-vb', dest='val_batch_size', metavar='VB', type=int, default=4,
                        help='Val_Batch size')
    parser.add_argument('--learning-rate', '-l', metavar='LR', type=float, default=1e-3,
                        help='Learning rate', dest='lr')
    parser.add_argument('--load', '-f', type=str, default="F:/MyReSegmet/pretrain/shufflenetv2_x1.pth",
...
if __name__ == '__main__':
    ...
    net = SUNet(n_channels=3, n_classes=args.classes)
