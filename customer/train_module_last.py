...
#
'''目前最新的一个版本'''
'''tensorbaord和wandb记录 只保存最新的和最好的权重'''
...
def train_net(net,
    ...
    args2.resize = (512, 256)  # 输入是w h(长和高)


    train_d = pascal_customer.Customer_VOCSegmentation(args2, split='train',isAug=True)
    ...
    # (Initialize logging)
    experiment = wandb.init(project='unetae', resume='allow', anonymous='must')
    experiment.config.update(dict(epochs=epochs, batch_size=batch_size, learning_rate=learning_rate,
                                  save_checkpoint=save_checkpoint
                                  ))

    # 初始化tensorboard
    writer = SummaryWriter(r"F:\MSegmentation\TensorboardLog")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if save_checkpoint:
            ...
            #     dir_checkpoint / 'checkpoint_epoch{}_valloss{:.2f}.pth'.format(epoch, total_train_loss)))  # 保存当前的
...
def get_args():
    ...
    parser.add_argument('--ignore-index', '-i', type=int, dest='ignore_index', default=100,
