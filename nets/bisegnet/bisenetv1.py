...
from nets.bisegnet.resnet import Resnet18
...
class AttentionRefinementModule(nn.Module):
    ...
    def forward(self, x):
        feat = self.conv(x) #先降维
        atten = torch.mean(feat, dim=(2, 3), keepdim=True)#全局池化
        atten = self.conv_atten(atten)#1*1
        atten = self.bn_atten(atten)#bn
        #  atten = self.sigmoid_atten(atten)
        atten = atten.sigmoid()#sigmod
...
class BiSeNetV1(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.aux_mode == 'train': #训练的时候两个辅助解码头
            feat_out16 = self.conv_out16(feat_cp8)
            feat_out32 = self.conv_out32(feat_cp16)
            return feat_out, feat_out16, feat_out32 #fo是原版的最终的
