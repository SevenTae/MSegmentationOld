...
class cbr(nn.Module):
    def __init__(self,inchannel,outchannel,ksize,padding,group=1):
        super(cbr, self).__init__()
        self.seq= nn.Sequential(
            nn.Conv2d(inchannel, outchannel, ksize, padding=padding,groups=group),
...
class DoubleDWConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None):
        ...
        self.double_dwconv = nn.Sequential(
            cbr(in_channels,in_channels,ksize=3,padding=1,group=in_channels),
            cbr(in_channels, mid_channels, ksize=1,padding=0),

            cbr(mid_channels, mid_channels, ksize=3, padding=1,group=mid_channels),
...
class encodeconv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None):
        ...
        self.conv = nn.Sequential(
            #1*1降维
            cbr(in_channels,out_channels,ksize=1,padding=0),
            # #深度可分离
            cbr(out_channels, out_channels, ksize=3, padding=1,group=out_channels),
            cbr(out_channels, out_channels, ksize=1, padding=0),
...
# moel = encodeconv(464,232 )
# inpp = torch.randn([1,464,64,64])
# out=moel(inpp)
# print(out.shape)
...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            self.conv = DoubleDWConv(in_channels, out_channels, in_channels // 2)
        else:
            self.up = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2)
            # self.conv = DoubleDWConv(in_channels, out_channels)
            self.conv=encodeconv(in_channels,out_channels)
...
class Up2(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, doublein=48, mid=24, bilinear=True):
        ...
        if bilinear:
            ...
            # self.conv = DoubleDWConv(doublein, out_channels)
            self.conv = encodeconv(doublein, out_channels)
